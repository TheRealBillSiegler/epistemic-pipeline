...
> — Richards J. Heuer Jr., *Improving Intelligence Analysis with ACH* (the verbatim source of this wording, confirmed by decoding the handout PDF; his 1999 *Psychology of Intelligence Analysis* makes the same point in slightly different words — "no guarantee… fallible *intuitive* judgment… *Analysis of Competing Hypotheses* does, however…")
...
- **Detecting *absence* is structurally harder than detecting conflict.** Spotting that evidence *disagrees* is anomaly detection over what's present; spotting that relevant evidence is *missing* requires quantifying over what *could* have been present. PassiveQA (arXiv:2604.04565, 2026) names the absence problem directly; even after fine-tuning, its abstain-recall only reaches 58.1% (up from 13.3% at inference time) — a hard, partly-solved problem. (The absence-vs-conflict contrast is our synthesis; the paper supplies the absence half.)
- **One-sided-but-TRUE evidence beats every falsehood check.** This is the sharpest result for us. *Epistemic Bias Injection* (arXiv:2512.00804, 2026): factually accurate but cherry-picked passages crowd out other views and steer the conclusion, and a fact-checker **cannot** tell them from balanced ones (most adversarial passages are both highly retrievable and perspective-biased — 74.7% of negative-bias and 78.0% of positive-bias injections satisfy *both* properties, ~90% satisfy at least one; fact-check "not enough info" rates 0.69 vs 0.71). **Omission-honesty cannot be reduced to truth-checking.** A belief can be fully sourced, every claim true, and still be a lie of selection. The pipeline needs an explicit *viewpoint-coverage / one-sidedness* check — and the literature offers no validated one yet. This is the open frontier.
...
**Body-level quote/number check (2026-06-26, deterministic — fetch the paper body + string-match, no model):** every verbatim quote and number above was re-matched against the actual paper body (arXiv HTML / PMC / ACL), closing the "checked against our summary, not the paper" gap for anything that is a literal string. Confirmed in-body: the conformal "completely sure about an incorrect answer" quote; Epistemic Bias Injection 74.7% / 78.0%; PassiveQA 58.1% / 13.3%; InfoGatherer's Deng-entropy / ignorance / nonspecificity and its abstain-when-turn-budget-exhausted rule; Tomani 92.5→99.4 / 92.1→95.1 (RLHF-conditional); RedDebate's 38.7 baseline. **Corrected by this pass:** the ">86% attack success" figure was *not* in the paper — replaced above with the real property-satisfaction rates (74.7% / 78.0% both, ~90% at least one). **Residuals from the first body pass, now closed:** Ren's 4% model-uncertainty F1 is confirmed in-body — "qwen3-8b achieves a respectable 69.8% du-f1 but a negligible 4.0% mu-f1" (the first pass's bare "4.0" needle had collided with the license string; a tighter needle resolves it). Gilardi's 25-pt accuracy gap and 97%-at-temperature-0.2 intercoder agreement were confirmed via the PMC mirror (PMC10372638), since PNAS blocks raw fetch.

**Heuer PDF closed with a real parser (2026-06-26):** the handout PDF that a naive tag-strip "could not decompress" decodes cleanly with `pypdf` (it was FlateDecode, not unreadable). The full process-vs-verdict quote, disconfirmation-first scoring, and diagnosticity are all present verbatim in the handout body. This surfaced an attribution fix applied above: our exact wording ("obviously… fallible *human* judgment… *ACH* does, however") is the **handout's**, not the 1999 **book's** (the book says the same thing with different words). No string-checkable claim now remains unverified against its source. Tools, re-runnable: `tools/quote_check.py` (HTML/text) and `tools/pdf_quote_check.py` (PDF, needs `uv run --with pypdf`).
