...
**Update (#25).** Root-keyed two-tier fusion (the [2026-06-26 design](2026-06-26-shared-source-root-keyed-fusion-design.md)) closes the *exact-duplicate* part of this residue. Every increment carries a canonical root id; the fold averages within a root and accumulates only across distinct roots, so a re-import or re-citation of one source no longer settles a belief more than that one source warrants. The *partially*-correlated middle case — a note that paraphrases a source but names no machine-readable origin — remains the residual, by that design's own §10.
